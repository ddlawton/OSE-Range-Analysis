...
```mermaid
flowchart TD
    A[Raw Data<br/>data/raw/] --> B[_targets.R Pipeline]
    B --> C[Data Processing<br/>Targets]
    C --> D[Statistical Models<br/>GLMMs & GAMs]
    D --> E[Model Objects<br/>data/model_objects/*.rds]
    D --> F[Summary Tables<br/>outputs/tables/*.csv]
    D --> G[Diagnostic Plots<br/>outputs/figures/*.png]
    E --> H[Quarto Documents<br/>R/analysis/*.qmd]
    F --> H
    G --> H
...
```
├── _targets.R                    # 🎯 MAIN PIPELINE - all computation happens here
├── _targets/                     # Pipeline metadata & object cache (git-ignored)
│   ├── meta/                     # Dependency tracking
│   └── objects/                  # Intermediate targets (binary)
├── R/
│   ├── _common.R                 # Shared setup, packages, configuration
│   ├── index.qmd                 # Main landing page
│   ├── analysis/                 # Analysis documents (.qmd)
...
│   └── functions/                # Modular R functions (sourced by _targets.R)
│       ├── data_preprocessing.R  # Data loading and cleaning
│       ├── statistical_models.R  # GLMM, GAM model fitting
│       ├── plotting_functions.R  # Publication-ready plots
│       ├── figure_export.R       # File saving utilities
│       ├── senegal_map.R         # Geographic mapping
│       └── table_variable_summary.R # Data summaries
├── data/
│   ├── raw/                      # Original Excel data file
│   ├── processed/                # Cleaned data (generated by targets)
│   └── model_objects/            # 💾 Model RDS files (generated by targets)
├── outputs/
│   ├── figures/                  # 📊 High-resolution figures (generated by targets)
│   └── tables/                   # 📋 CSV tables (generated by targets)
├── _site/                        # Rendered website (git-ignored)
├── _quarto.yml                   # Website configuration
└── .github/workflows/            # Automated testing & deployment
    └── quarto-publish.yml        # CI/CD with targets caching
```
...
3. **Run the targets pipeline:**
   ```r
   # Execute all computational targets (models, summaries, figures)
   targets::tar_make()
...
**Quarto Integration:**
- Analysis documents load artifacts from `data/model_objects/` and `outputs/`
- No computational work happens in `.qmd` files
- Fast re-renders when only presentation changes
