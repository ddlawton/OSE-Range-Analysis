...
The goal of this page is to replicate the current figures in the manuscript. This is to ensure visuals for publication have the same themeing but also to ensure that my data management aligns well with expectations and that there are no errors. If you see errors, please let me know!
...
\text{ose count}_{ij} =\ 
  ...
  &+ u_j \\
  &+ \varepsilon_{ij}
\end{align*}ilon\_{ij} \\end{align\*}ilon\_{ij} \\end{align\*} 
$$
...
#| message: false
#| warning: false
#| fig-width: 5       # device width in inches
#| fig-height: 10      # device height in inches
#| fig-retina: 2      # scale up for high-DPI (keeps the output sharp)
